...
/*四轮四驱*/  
   ...
   ROS_INFO("-------------");
    ...
    z_mid_angle =msg.angular.z;//将这个值作为Z旋转时的速度目标
    z_mid_speed =msg.linear.z;//将这个值作为Y方向的速度目标
    ...
    if(z_mid_angle < -1.1)z_mid_angle = -1.1;
            ...
            if(x_mid_speed>0 && z_mid_speed==0 && z_mid_angle==0){//按下 I 键 
				       ...
				       angle_A=+90;angle_B=+90;angle_C=+90;angle_D=+90;
			               ...
			               speed_A= z_mid_angle/2;
				       speed_B= -z_mid_angle/2; 
				       speed_C= -z_mid_angle/2; 
				       speed_D= z_mid_angle/2;  
...
int main(int argc, char **argv){
   ...
   while(ros::ok())
   {
	 ...
	 float accelerated_speed_x = Data_UR[12]/16384;//转换成 g	,重力加速度定义为1g, 等于9.8米每平方秒
	 float accelerated_speed_y = Data_UR[13]/16384;//转换成 g	,重力加速度定义为1g, 等于9.8米每平方秒
	 float accelerated_speed_z = Data_UR[14]/16384;//转换成 g	,重力加速度定义为1g, 等于9.8米每平方秒
            ...
            if(n>0)
               {
                   ...
                   uint8_t buffer[92];
                   for(uint8_t i=0;i<n;i++){if(buffer[0]!=0XAA)sp.read(buffer, 1);else {n = sp.available();break;} }//读出数据

                   if(buffer[0]==0XAA && n>=92)
                     {
                       sp.read(buffer, 92);//读出数据
...
//                          ROS_INFO("[01] Flag_start: [%u ]", (uint8_t)Data_UR[0]);ROS_INFO("[01] Flag_start: ON");}//下位机电机启动/停止标志，1启动，0停止
//                      if((uint8_t)Data_UR[0]==0){
//                        ROS_INFO("[01] Flag_start: [%u ]", (uint8_t)Data_UR[0]);ROS_INFO("[01] Flag_start: OFF");}//下位机电机启动/停止标志，1启动，0停止
